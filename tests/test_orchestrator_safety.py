...
"""
    ...
    def test_micro_probe_shrink_rejects_retarget_outside_original_boundary(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "targeted SPEC micro-probe rewrite changed target_regions "
                "from ['target.py::parse_item'] to ['target.py::other_item']",
                issues,
            )

    def test_micro_probe_shrink_rejects_allowed_region_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            agent = MicroAgent(
                {
            ...
            self.assertIn(
                "targeted SPEC micro-probe rewrite changed allowed_regions "
                "from ['target.py::parse_item'] to ['target.py::other_item']",
                issues,
            )

    def test_micro_probe_shrink_rejects_expected_region_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            agent = MicroAgent(
                {
            ...
            self.assertIn(
                ...
                "expected_changed_regions from ['target.py::parse_item'] "
                "to ['target.py::other_item']",
                issues,
            )

    def test_micro_probe_shrink_falls_back_to_target_regions_for_contract_regions(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
    ...
    def test_micro_probe_shrink_allows_multi_region_task_to_choose_one_region(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
    ...
    def test_micro_probe_shrink_rejects_multi_region_task_to_unknown_region(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            ...
            self.assertIn(
                "targeted SPEC micro-probe rewrite changed target_regions "
                "from ['target.py::parse_item', 'target.py::format_item'] "
                "to ['target.py::other_item']",
                issues,
            )

    def test_micro_probe_shrink_rejects_build_kernel_to_build_shift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            agent = MicroAgent(
                {
                ...
                "task_graph": [
                    {
                        ...
                        "deliverables": ["perf_takehome.py"],
                        "edit_scope": "Add one guard inside build",
                        "probe_diff_contract": {
                            "allowed_files": ["perf_takehome.py"],
                            "allowed_regions": ["perf_takehome.py::KernelBuilder.build"],
                            "expected_changed_regions": [
            ...
            self.assertIn(
                "targeted SPEC micro-probe rewrite changed target_regions "
                "from ['perf_takehome.py::KernelBuilder.build_kernel'] "
                "to ['perf_takehome.py::KernelBuilder.build']",
                issues,
            )
...
END_HYPOTHESIS_OPTION"""
    ...
    def test_simple_mode_rejects_reasoning_only_thinking_brief_by_default(self) -> None:
        async def run_case() -> None:
            with tempfile.TemporaryDirectory() as tmp:
                repo = Path(tmp)
                (repo / "target.py").write_text("def parse_item(value):\n    return value\n")
    ...
    def test_simple_mode_can_opt_in_to_reasoning_only_thinking_brief(self) -> None:
        async def run_case() -> None:
            with tempfile.TemporaryDirectory() as tmp:
                repo = Path(tmp)
                (repo / "target.py").write_text("def parse_item(value):\n    return value\n")
            ...
            agent = MicroAgent(
                {
                    ...
                    "providers": {},
                    "mcp_servers": {},
                    "workflow": {"preset": "simple", "simple_report_enabled": True},
                },
                AgentState(repo_root=repo, user_request="test"),
            )
...
""",
    ...
    def test_mixed_noop_and_target_miss_prefers_current_source_next_improvement(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            target = repo / "target.py"
            target.write_text("value = 'fast'\n")
...
<candidate id="mixed">
<strategy_axis>general_edit</strategy_axis>
<reason>Re-apply one edit and miss another stale target.</reason>
<change>
<path>target.py</path>
<search>
value = 'fast'
...
""",
            ...
            self.assertIn("made no effective code change", repair_prompt)
            self.assertIn("Do not return the same replacement again", repair_prompt)
            self.assertNotIn("search/target block did not match", repair_prompt)
            record = json.loads(
                (repo / ".local_micro_agent" / "candidates.jsonl")
                .read_text()
                .splitlines()[0]
            ...
            self.assertEqual(record["repair_parent_patch_miss_kind"], "target_not_found")
            kinds = {
                event["patch_miss_kind"]
                for event in record.get("repair_parent_patch_miss_events", [])
            }
            self.assertEqual(kinds, {"patch_noop", "target_not_found"})
